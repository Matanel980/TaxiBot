...
export function CollapsibleDashboardSheet({
  ...
  // maxDrag is already calculated above (line 45)
  // Calculate drag constraints: sheet can move from 0 (fully expanded) to maxDrag (collapsed)

  return (
    <motion.div
      ref={sheetRef}
      className="fixed bottom-0 left-0 right-0 z-10 bg-gray-900/95 backdrop-blur-xl rounded-t-3xl shadow-2xl overflow-hidden"
      style={{
        y: springY,
    ...
    >
      ...
      >
        {/* Visual drag handle */}
        <div className="w-12 h-1.5 bg-gray-500/60 rounded-full mx-auto mb-2" />
